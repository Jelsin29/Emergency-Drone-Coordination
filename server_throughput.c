...
void init_perf_monitor(const char* log_filename) {
    ...
    if (log_filename) {
        ...
        if (metrics.log_file) {
            // Write CSV header
            fprintf(metrics.log_file, "timestamp,elapsed_seconds,total_messages,msg_per_sec,status_updates,missions,heartbeats,errors,active_connections,total_bytes_rx,total_bytes_tx,avg_response_ms,max_response_ms,peak_msg_per_sec\n");
...
double get_elapsed_seconds(void) {
    ...
    return (now.tv_sec - metrics.start_time.tv_sec) + 
           (now.tv_nsec - metrics.start_time.tv_nsec) / 1000000000.0;
}
...
void log_perf_metrics(void) {
    ...
    double avg_response = metrics.response_count > 0 ? 
                         metrics.total_response_time_ms / metrics.response_count : 0;
    ...
    printf("Messages: %lu total (%.2f msgs/sec, peak: %lu msgs/sec)\n", 
           metrics.messages_processed, msg_rate, metrics.peak_messages_per_second);
    ...
    printf("Connections: %lu active, %lu total, %lu disconnected (peak: %lu)\n",
           ...
           metrics.disconnections, metrics.peak_connections);
    
    printf("Data Transfer: %.2f KB received, %.2f KB sent\n",
           metrics.total_bytes_received / 1024.0, metrics.total_bytes_sent / 1024.0);
    
    if (metrics.response_count > 0) {
        printf("Response Times: avg %.2fms, min %.2fms, max %.2fms\n",
               avg_response, metrics.min_response_time_ms, metrics.max_response_time_ms);
    }
...
void log_perf_metrics_to_file(void) {
    ...
    fprintf(metrics.log_file, "%s,%.2f,%lu,%.2f,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%.2f,%.2f,%lu\n",
            timestamp, elapsed, metrics.messages_processed, msg_rate,
            metrics.status_updates_received, metrics.missions_assigned, 
            metrics.heartbeats_sent, metrics.error_count, metrics.active_connections,
            metrics.total_bytes_received, metrics.total_bytes_sent,
            avg_response, metrics.max_response_time_ms, metrics.peak_messages_per_second);
...
void export_metrics_json(const char* filename) {
    ...
    fprintf(json_file, "    \"min_response_time_ms\": %.2f\n", 
